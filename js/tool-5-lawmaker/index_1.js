...
===INSTRUCTION
Write a concise summary of [KASUS HUKUM]. Preserve all names,places,dates,tools,weapons. Don't write anything else.
`
...
===INSTRUCTION
Attach all Pasal-pasal applicable to each actor in the law case
...
async function bookSearch_buku_bab(searchTerm='', div_search_result_id)
{
  ...
  if (match) 
  {
    ...
    docById('div_search_results').innerHTML = "";
    for (const tc of kb_kuhp.data.textChunks) 
    {
      if( tc.keyConcepts[0].startsWith(buku) && tc.keyConcepts[1].startsWith(bab) )
      {
        ...
        docById('div_search_results').innerHTML += res;
        isFound = true;
      }
    }
    return isFound;
    ...
    html: `
        <div id="container_search_${wndId}" class_="row">
          ...
          <div id="div_search_bar_${wndId}" class="div_search_bar mb-3 hstack">  
              <input style="width:90%;" id="${tb_search_term_id}" value="${searchTerm}">
...
const wnd_kasus_analisa = new WinBox({
  ...
  height: "40%",
  html: `
      <div id="container_kasus_analisa" class_="col">
        <textarea style="width:100%;height:100%;" id="ta_kasus_analisa"></textarea>
      </div>        
    `
  });
            ...
            <div class="row g-1" style="padding:0.3em;height:100%;">
              <div class="col-2" style="overflow-x:clip;">
                ...
                <ul id="ul_book_sections" class="list-group list-group-flush">
                  <li class="list-group-item">Kesatu</li>
                  <li class="list-group-item">Kedua</li>
                  <li class="list-group-item">Ketiga</li>
                </ul>
              </div>
              ...
              <div class="col-6" style="">
                PASAL-PASAL
                <div id="ta_book_section_item_content" style="width:100%;height:100%;resize: none!important;"></div>
              </div>
            </div>
        `
      });
...
onload = async function () {
  ...
  //Search stuff
  createWndSearch("kejahatan terhadap anak")
